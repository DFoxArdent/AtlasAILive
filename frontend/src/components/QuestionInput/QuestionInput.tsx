...
export const QuestionInput = ({
  ...
  const sendQuestion = async () => {
    if (disabled || (!question.trim() && !base64Image && !documentFile)) return;
    ...
    if (base64Image) {
      ...
      if (documentChunks && documentChunks.length > 0) {
        const segments = documentChunks.map((chunk) => ({ type: 'text', text: chunk }));
        questionContent = question.trim()
          ? JSON.stringify([{ type: 'text', text: question }, ...segments])
          : JSON.stringify(segments);
      } else {
        questionContent = question.toString();
      }
    } else {
      questionContent = question.toString();
    }
